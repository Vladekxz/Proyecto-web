  ...
  if(skus===null){
    ...
    function crearEventoEliminar(producto, sku) {
      return function(event) {
        ...
        var subtotalElement = productoElement.getElementsByClassName('subtotal')[0];
        var subtotal = parseInt(subtotalElement.textContent);
  
        if (skuCounters[sku] > 0) {
          skuCounters[sku] -= 1;
          total -= producto.precio;
          cantidadElement.textContent = skuCounters[sku];
          subtotalElement.textContent = subtotal - producto.precio;
          var storage = localStorage.getItem('cartItems');
          var skus = JSON.parse(storage);
